...
with open(os.path.join(OUTPATH, "tmp.typst"), "w") as f:
    f.write(header)
    for c in components:
        f.write(read_section_from_yaml(c))


compilation_result = subprocess.run(f"typst compile {OUTPATH}/tmp.typst", shell=True, capture_output=True, text=True)
