...
def displayPDF(filename):
    ...
    pdf_path = os.path.join(OUTPATH, f"{filename}.pdf")
...
section_titles = [
    file for file in os.listdir(DATAPATH) if file.endswith(f"_{locale}.yaml")
]
...
with preview_col:
    filename_col, compile_col = st.columns([4, 2], vertical_alignment="bottom")
    ...
    with compile_col:
        st.button(
            "compile",
            icon="🔄",
            on_click=lambda: write_and_compile_document(sections, filename=filename),
        )
